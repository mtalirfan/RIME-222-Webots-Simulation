...
front_distance_threshold = 600  # 450,
left_right_distance_threshold = 500  # 400,
left_right_wall_threshold = 650  # 450,
left_right_wall_crit_threshold = 800  # 550,
front_wall_threshold = 700  # 500,
wall_collide_threshold = 1000  # 950,

max_velocity = 10
turn_coefficient = -1
soft_turn_coefficient = -1
sharp_turn_coefficient = -1
...
while robot.step(timestep) != -1:
    ...
    if (
        ...
        robot_move("forward")
        if leftDsVal > left_right_wall_crit_threshold:  # too close to left wall
            robot_move("right")
            if frontLeftDsVal < left_right_wall_threshold:
                robot_move("forward")
        elif leftDsVal > left_right_wall_threshold:  # close to left wall
            robot_move("soft right")
            if frontDsVal < left_right_wall_threshold:
                robot_move("forward")
        if rightDsVal > left_right_wall_crit_threshold:  # too close to right wall
            robot_move("left")
            if frontRightDsVal < left_right_wall_threshold:
                robot_move("forward")
        elif rightDsVal > left_right_wall_threshold:  # close to right wall
