...
front_distance_threshold = 400
left_right_distance_threshold = 300
left_right_wall_threshold = 500
front_wall_threshold = 500
...
def robot_move(direction="forward"):
    match direction:
        ...
        case "back":
            ...
            rmotor.setVelocity(-max_velocity)
        case "left":
            lmotor.setVelocity(0.7 * max_velocity)
            rmotor.setVelocity(1.3 * max_velocity)
        case "right":
            lmotor.setVelocity(1.3 * max_velocity)
            rmotor.setVelocity(0.7 * max_velocity)
        case "soft left":
            lmotor.setVelocity(0.85 * max_velocity)
            rmotor.setVelocity(1.15 * max_velocity)
        case "soft right":
            lmotor.setVelocity(1.15 * max_velocity)
            rmotor.setVelocity(0.85 * max_velocity)
        case "stop":
...
while robot.step(timestep) != -1:
    ...
    if (
        ...
        # or leftFrontLeftDsVal > left_right_wall_threshold
    ):  # too close to left wall
        robot_move("soft right")
        if frontLeftDsVal < left_right_wall_threshold:
            robot_move("forward")
    ...
    if (
        ...
        if frontRightDsVal < left_right_wall_threshold:
            robot_move("forward")

    if (
        rightFrontRightDsVal < left_right_distance_threshold
        or leftFrontLeftDsVal >= left_right_distance_threshold
        # and abs(rightDsVal - leftDsVal) > left_right_distance_threshold
    ):  # there is way on right, or no way on left, and right way is much more than left
        robot_move("right")

    if (
        leftFrontLeftDsVal <= left_right_distance_threshold
        or rightFrontRightDsVal > left_right_distance_threshold
        # and abs(rightDsVal - leftDsVal) > left_right_distance_threshold
    ):  # there is way on left, or no way on right, and left way is much more than right
        robot_move("left")
